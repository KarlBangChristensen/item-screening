...
$A$ and $B$ are variables that correspond to distinct items if we check for LD. If we check for DIF, one is an item and one is an exogenous variable. Pairs for which the null hypothesis is rejected are flagged before proceeding to the next step.
...
\State Compute
\coauthortodo{\textbf{Asymptotic $p$-values.} Algorithm~\ref{alg:part_gam} returns only $\gamma$, but later steps require $p$-values. Please document the variance estimate, tail calculation, and handling of undefined tests actually used by \texttt{twigg}, then update the algorithm's outputs. Check consistency with the reported results; DIGRAM equivalence has not been verified.}%
\[
...
\coauthortodo{\textbf{Zero weights.} Algorithm~\ref{alg:part_gam} returns $\mathrm{NA}$ when a direction has no comparable pairs. Then $0\cdot\mathrm{NA}$ makes this weighted expression undefined, even if the other direction is usable. Please specify how zero-weight terms are handled.}%
\[
WPG_{ii'}
=
\frac{
...
\While{$\mathcal{H} \neq \emptyset$}
    ...
    \State Using only the currently active significant directional hypotheses, partition
    ...
    \State First, consider pairs with two active significant partial gamma coefficients:
    \coauthortodo{\textbf{Mixed signs.} Two significant directions of opposite sign belong to none of the four lists. The loop can then seek a maximum over an empty set while $\mathcal{H}$ is nonempty. Please define their treatment.}%
    \[
    \mathcal{L}^{(2)}
    =
    \mathcal{L}_{++}^{(2)}
    ...
    \If{$\mathcal{L}^{(2)} \neq \emptyset$}
        ...
        pairs with two active significant partial gamma coefficients:
        \[
        \{Y_a,Y_b\}
        =
    ...
    \Else
        \State Select the pair with the largest absolute weighted partial gamma among
        pairs with only one active significant partial gamma coefficient:
        \[
        \{Y_a,Y_b\}
        =
