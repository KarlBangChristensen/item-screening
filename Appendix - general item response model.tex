...
$$
is a normalizing constant that does not depend on the observed data $(\boldsymbol{y},\boldsymbol{x})$. Note that this reduces to the dichotomous normalizing constant of the main text when $M=1$ and that $S$ is also sufficient in this general model.

%\coauthortodo{\textbf{Ordinal notation.} Please check $\alpha_i$ below against the category-specific $\alpha_{iy_i}$ above. Also distinguish the counts $n_L,n_G$ from the collections $L,G$.}%
...
\begin{align}
P(\mathbf{Y}=\mathbf{y}\mid \Theta=\theta,\mathbf{X}=\mathbf{x}) = \exp\Big(\alpha_0+\theta S+\sum_{i=1}^k\alpha_{iy_i}+(\star)\Big),
\end{align}
where
\begin{align}
(\star)=\sum_{i=1}^{n_L}\lambda_i(u_i,v_i)+ \sum_{i=1}^{n_D}\delta_i(a_i,z_i)+\sum_{i=1}^{n_G}\mu_i(g_i).
\end{align}
Here $n_D$ denotes the total number of DIF pairs $\boldsymbol{D}=(D_1,\dots,D_{n_D})$ with $D_d=(A_d,Z_d)$, for an item $A_d\in\{Y_1,\dots,Y_k\}$, and a covariate $Z_d\in\{X_1,\dots,X_m\}$, where $Z_d$ is a source of DIF for $A_d$. Furthernore $n_L$ denotes the total set of LD pairs $\boldsymbol{L}=(L_1,\dots,L_{n_L})$ with $L_d=(U_d,V_d)$, where $U_d,V_d\in\{Y_1,\dots,Y_k\}$. Lastly, $n_G$ is the total number of interactions with more complex DIF or LD structure, $\boldsymbol{G}=(G_1,\dots,G_{n_G})$ with $G_c\subset \{Y_1,\dots,Y_k,X_1,\dots,X_m\}$.
