...
We apply the screening procedure to data from $99$ respondents who completed the $10$ item version of Actinic Keratosis Quality of Life (AKQoL) questionnaire \cite{Esmann-2013}. Items $Q1$--$Q10$, each scored $0$--$3$ along with two covariates were also recorded: sex (\texttt{woman}, $1=$ woman) and age (\texttt{Alder}, in years). All $99$ respondents have complete item and covariate data.
...
\subsection{Step 4}
Backward elimination retains neither covariate as a structural predictor of the score: conditioned on the other, both \texttt{woman} and \texttt{Alder} fail to reach significance, and \texttt{woman}'s marginal association does not support criterion validity either.
\begin{table}[H]
\centering
\begin{tabular}{@{}lrrll@{}}
\toprule
...
\midrule
woman & $0.357$ & $0.105$ & Alder & FALSE \\
Alder & $-0.054$ & $0.502$ & woman & FALSE \\
\midrule
woman (final) & $0.338$ & NA & None & FALSE \\
\bottomrule
\end{tabular}
...
\subsection{Step 5}
Collecting the Step 3 and Step 4 results gives the tentative GLLRM: a measurement edge $\theta\to Q_i$ for each item, DIF edges Alder$\,\to Q4$ and woman$\,\to Q5$, and LD edges $(Q3,Q10)$ and $(Q5,Q6)$. Because neither covariate supported criterion validity in Step 4, the graph includes no score-covariate association; \texttt{Alder} and \texttt{woman} enter the graph only through their DIF edges on $Q4$ and $Q5$, respectively.
\begin{table}[H]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
...
\end{table}
Both DIF edges survive their $C5$ tests: Alder$\,\to Q4$ ($p=0.0031$) and woman$\,\to Q5$ ($p=0.0008$). Both LD pairs survive, each with at least one significant conditioning direction. The final GLLRM for the AK data therefore retains all $10$ items, two genuine DIF effects (Alder on $Q4$ and woman on $Q5$), and two genuine LD pairs ($Q3$--$Q10$ and $Q5$--$Q6$); neither covariate supported criterion validity in Step 4, so the model includes no score-covariate association.
