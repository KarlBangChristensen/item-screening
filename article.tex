...
We apply the screening procedure to data from $99$ respondents who completed the $10$ item version of Actinic Keratosis Quality of Life (AKQoL) questionnaire \cite{Esmann-2013}. Items $Q1$--$Q10$, each scored $0$--$3$ along with two covariates were also recorded: sex (\texttt{gender}, $1=$ woman) and age (\texttt{age}, in years). All $99$ respondents have complete item and covariate data.
...
\midrule
$Q4$ & age & $0.383$ & $0.0031$ & $0.031$ & * \\
$Q5$ & gender & $0.727$ & $0.0008$ & $0.017$ & * \\
\bottomrule
\end{tabular}
...
\midrule
$Q4$ & age & $0.383$ & $0.0031$ & DIF \\
$Q5$ & gender & $0.727$ & $0.0008$ & DIF \\
\bottomrule
\end{tabular}
...
\subsection{Step 4}
Backward elimination retains neither covariate as a structural predictor of the score: conditioned on the other, both \texttt{gender} and \texttt{age} fail to reach significance, and \texttt{gender}'s marginal association does not support criterion validity either.
\begin{table}[H]
\centering
\begin{tabular}{@{}lrrll@{}}
\toprule
...
\midrule
gender & $0.357$ & $0.105$ & age & FALSE \\
age & $-0.054$ & $0.502$ & gender & FALSE \\
\midrule
gender (final) & $0.338$ & NA & None & FALSE \\
\bottomrule
\end{tabular}
\caption{Step 4 backward elimination search (asymptotic $p$-values). $\mathcal{C}$ is the conditioning set; S.C.V.\ abbreviates ``supports criterion validity''.}
\label{tab:ak_step4}
\end{table}

\subsection{Step 5}
Collecting the Step 3 and Step 4 results gives the tentative GLLRM: a measurement edge $\theta\to Q_i$ for each item, DIF edges age$\,\to Q4$ and gender$\,\to Q5$, and LD edges $(Q3,Q10)$ and $(Q5,Q6)$. Because neither covariate supported criterion validity in Step 4, the graph includes no score-covariate association; \texttt{age} and \texttt{gender} enter the graph only through their DIF edges on $Q4$ and $Q5$, respectively.
\begin{table}[H]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
...
\midrule
age $\to Q4$ & $C5$ & $0.383$ & $0.0031$ & Retained \\
gender $\to Q5$ & $C5$ & $0.727$ & $0.0008$ & Retained \\
\bottomrule
\end{tabular}
...
\end{table}
Both DIF edges survive their $C5$ tests: age$\,\to Q4$ ($p=0.0031$) and gender$\,\to Q5$ ($p=0.0008$). Both LD pairs survive, each with at least one significant conditioning direction. The final GLLRM for the AK data therefore retains all $10$ items, two genuine DIF effects (age on $Q4$ and gender on $Q5$), and two genuine LD pairs ($Q3$--$Q10$ and $Q5$--$Q6$); neither covariate supported criterion validity in Step 4, so the model includes no score-covariate association.


\section{Discussion}

% \coauthortodo{\textbf{Subscore count.} Excluding the empty subset would give $2^{k-1}-1$. Please check the count in this paragraph.\\ \textcolor{blue}{Change accepted and implemented by STK. Andreas' comment is correct.}}%
...
The AK data illustrate a further choice tied to negative local dependence. The $(Q3,Q10)$ pair has a negative weighted partial gamma, and our stepwise inclusion algorithm retains it as genuine LD in Step 3(a) on the same footing as a positive pair. DIGRAM's SCREEN J procedure instead requires the sum of the two directed partial gammas to be strictly positive before a pair enters the model; a pair such as $(Q3,Q10)$ that fails this criterion is flagged as negative LD; sometimes difficult to interpret or indicative of other problems with the measurement model, and is excluded from the screened model rather than retained. Whether to apply this positivity rule is therefore another choice that can change the composition of the final GLLRM.
...
A related choice concerns how the Benjamini--Hochberg correction is applied in Step 2. We compute separate false-discovery-rate thresholds for the LD and DIF test families, whereas DIGRAM's SCREEN J procedure pools all local-dependence and DIF partial-gamma $p$-values into a single combined family before computing the threshold \citep{kreiner-christensen-2011}. Pooling yields a stricter, more conservative cutoff and can change which relations are initially flagged as candidates. Researchers need to decide whether to pool LD and DIF $p$-values into one combined Benjamini--Hochberg family or treat them as separate families, and this choice should be reported explicitly, since it can materially affect the screening result.
